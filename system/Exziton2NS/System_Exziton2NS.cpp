...
class System : public System_Parent{
    public:
        ...
        bool init_system() {
            ...
            if (std::abs(temp.sum()) >= 1E-10){
                logs("Unitary timetransformation does not match matrix exponential! MaxCoeff = {} - May cause problems!\n\n",std::abs(temp.sum()));
        ...
        MatrixXcd dgl_timetrafo(const MatrixXcd &A, const double t) const {
            ...
            if (parameters.doInteractionPicture == 1) {
                ...
                if (parameters.orderTimeTrafo == TIMETRANSFORMATION_PRECALCULATED) {
                    int i,j,pn,pm;
                ...
                else if (parameters.orderTimeTrafo == TIMETRANSFORMATION_MATRIXEXPONENTIAL) {
                    MatrixXcd U = (1i*operatorMatrices.H_0*t).exp(); // auto slower than direct type, because of doubled evaluation
                    ret = U*A*U.conjugate();
                } else if (parameters.orderTimeTrafo == TIMETRANSFORMATION_MATRIXEXPONENTIAL_PRECALCULATED) {
