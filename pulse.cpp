...
void Pulse::generate( Parameters &p ) {
    ...
    if ( inputs.pulse_type.compare( "gauss_pi" ) == 0 ) {
        inputs.pulse_type = "gauss";
    }
    ...
    for ( double t1 = p.t_start; t1 < p.t_end + p.t_step * steps.size(); t1 += step ) {
        for ( int i = 0; i < (int)steps.size(); i++ ) {
            ...
            if ( inputs.pulse_type.compare( "cw" ) == 0 && t >= p.pulse_center )
                val = std::exp( -1i * p.pulse_freq * ( t - p.pulse_center ) );
            else if ( inputs.pulse_type.compare( "gauss" ) == 0 )
...
void Pulse::generateFromParam( Parameters &p ) {
    ...
    inputs.pulse_type = p.pulse_type;
    generate( p );
    logs.level2( "done!\n" );
}
