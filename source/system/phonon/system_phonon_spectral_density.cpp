...
double System::dgl_phonons_spectral_density( const double w ) {
    if ( parameters.p_phonon_qd_ae == 0.0 ) {
        ...
        return parameters.p_phonon_alpha * std::pow(w,parameters.p_phonon_ohm) * gaussian_shape;
    } else {
        ...
        return std::pow(w,parameters.p_phonon_ohm) * parameters.hbar * std::pow( parameters.p_phonon_qd_de * gaussian_shape_electron - parameters.p_phonon_qd_dh * gaussian_shape_hole, 2. ) / normalized;
    }
}
