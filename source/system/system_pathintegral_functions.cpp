...
Scalar System::dgl_phonon_S_function( const int t_delta, const int i_n, const int j_n, const int i_nd, const int j_nd ) {
    Scalar result = 0.0;
    if ( i_n == i_nd )
        result -= phi_vector_int[t_delta] * operatorMatrices.phonon_group_index_to_coupling_value[i_n];
    if ( j_n == j_nd )
        result -= std::conj( phi_vector_int[t_delta] ) * operatorMatrices.phonon_group_index_to_coupling_value[j_n];
    if ( i_n == j_nd )
        result += std::conj( phi_vector_int[t_delta] ) * operatorMatrices.phonon_group_index_to_coupling_value[i_n];
    if ( j_n == i_nd )
        result += phi_vector_int[t_delta] * operatorMatrices.phonon_group_index_to_coupling_value[j_n];
    return result;
}
