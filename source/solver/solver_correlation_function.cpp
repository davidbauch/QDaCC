...
#pragma omp parallel for schedule( dynamic ) shared( timer ) num_threads( s.parameters.numerics_maximum_threads )
    for ( long unsigned int i = 0; i < savedStates.size(); i += s.parameters.iterations_t_skip ) {
        ...
        for ( long unsigned int j = 0; j < savedRhos.size(); j++) {
            ...
            cache_noneq.at(i/s.parameters.iterations_t_skip).emplace_back( SaveScalar( s.dgl_expectationvalue<MatType, Scalar>( savedRhos.at(j).mat, op_creator, t_tau ), t_t, t_tau ) );
        }
    }
...
bool ODESolver::calculate_g2( System &s, const MatType &op_creator_1, const MatType &op_annihilator_1, const MatType &op_creator_2, const MatType &op_annihilator_2, Dense &cache, std::string purpose ) {
    ...
    ProgressBar progressbar = ProgressBar( totalIterations );
    timer.start();
    logs.level2( "Calculating G2(tau)... purpose: {}, saving to matrix of size {}x{}... ", purpose, dim, dim ); ////cache.size(), cache.at(0).size()
...
#pragma omp parallel for schedule( dynamic ) shared( timer ) num_threads( s.parameters.numerics_maximum_threads )
    for ( long unsigned int i = 0; i < savedStates.size(); i += s.parameters.iterations_t_skip ) {
        ...
        for ( long unsigned int j = 0; j < savedRhos.size(); j++) {
            double t_tau = savedRhos.at(j).t;
            cache_noneq.at(i/s.parameters.iterations_t_skip).emplace_back( SaveScalar( s.dgl_expectationvalue<MatType, Scalar>( savedRhos.at(j).mat, evalOperator, t_tau ), t_t, t_tau ) );
        }
    }
    // Because G1,G2 are better to be calculated on an equidistant grid, we interpolate the values to fit the grid.
    // The grid is determined by the initial timestep chosen (+ skip step)
