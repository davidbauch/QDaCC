...
MatType ODESolver::getHamilton( System &s, const double t, bool use_saved_hamiltons ) {
    if ( s.parameters.numerics_use_saved_hamiltons || use_saved_hamiltons ) {
        if ( savedHamiltons.size() == 0 || t > savedHamiltons.back().t ) {
            ...
            int approx = std::floor( t / s.parameters.t_step / 2.0 );
...
int ODESolver::reset( System &s ) {
    ...
    dim = (int)( s.parameters.iterations_t_max / s.parameters.iterations_t_skip ) + 10;
    return dim;
}
...
bool ODESolver::calculate_t_direction( System &s ) {
    ...
    saveState( rho, s.parameters.t_start, savedStates );
    s.expectationValues( rho, s.parameters.t_start, savedStates );

    // Main Time Loop
    for ( double t_t = s.parameters.t_start + s.parameters.t_step; t_t < s.parameters.t_end; t_t += s.parameters.t_step ) {
        ...
        if ( !s.traceValid( rho, t_t ) ) {
            t_t = s.parameters.t_end + s.parameters.t_step;
            return false;
        }
    }
...
#pragma omp parallel for schedule( dynamic ) shared( timer ) num_threads( s.parameters.numerics_maximum_threads )
    for ( int spec_w = 0; spec_w < s.parameters.iterations_w_resolution; spec_w++ ) {
        ...
        expfunc.reserve( savedStates.size() / s.parameters.iterations_t_skip + 1 );
        for ( int spec_tau = 0; spec_tau < ( s.parameters.t_end - s.parameters.t_start - 0.0 ) / ( s.parameters.t_step * s.parameters.iterations_t_skip ); spec_tau++ ) {
        ...
        for ( int i = 0; i < (int)savedStates.size(); i += s.parameters.iterations_t_skip ) {
            ...
            double t_t = getTimeAt( i );
            for ( int spec_tau = 0; spec_tau < ( s.parameters.t_end - s.parameters.t_start - t_t ) / ( s.parameters.t_step * s.parameters.iterations_t_skip ); spec_tau++ ) {
...
bool ODESolver::calculate_advanced_photon_statistics( System &s, const MatType &op_creator_1, const MatType &op_annihilator_1, const MatType &op_creator_2, const MatType &op_annihilator_2, std::string fileOutputName ) {
    ...
    if ( s.parameters.numerics_calculate_g2_C ) {
        ...
        //#pragma omp parallel for schedule( dynamic ) shared( timer ) num_threads( s.parameters.numerics_maximum_threads )
        for ( int T = 0; T < (int)savedStates.size(); T += s.parameters.iterations_t_skip ) {
    ...
    if ( !calculate_concurrence_with_g2_of_zero ) {
        ...
        timer.start();
        int tstart = output_full_ind ? 0 : (int)savedStates.size() - s.parameters.iterations_t_skip;
#pragma omp parallel for schedule( dynamic ) shared( timer ) num_threads( s.parameters.numerics_maximum_threads )
        for ( int T = tstart; T < (int)savedStates.size(); T += s.parameters.iterations_t_skip ) {
            Scalar top_1 = 0;
            Scalar top_2 = 0;
            Scalar bottom_1 = 0;
            Scalar bottom_2 = 0;
            for ( int i = 0; i < T; i += s.parameters.iterations_t_skip ) {
                ...
                for ( int j = 0; j < T - i; j += s.parameters.iterations_t_skip ) { //&& j + i < (int)savedStates.size()
    ...
    if ( output_full_g2 ) {
        ...
        for ( int i = 0; i < (int)savedStates.size(); i += s.parameters.iterations_t_skip ) {
            int k = i / s.parameters.iterations_t_skip;
            for ( int j = 0; j < (int)savedStates.size(); j += s.parameters.iterations_t_skip ) {
                int l = j / s.parameters.iterations_t_skip;
