...
void QDACC::Numerics::ODESolver::calculate_timebin_g3_correlations( System &s, const std::string &s_op_i, const std::string &s_op_j, const std::string &s_op_k, const std::string &s_op_l, const std::string &s_op_m,
                                                                    const std::string &s_op_n, const std::string &purpose, double t_start, double time_bin_length ) {
    ...
    for ( size_t i = 0; i < s.parameters.grid_values.size(); i++ ) {
        ////std::cout << s.parameters.grid_values[i] << ", ";
        if (i % 10 == 0) {
            ////std::cout << std::endl;
        }
    }
...
#pragma omp parallel for schedule( dynamic ) shared( timer ) num_threads( s.parameters.numerics_maximum_primary_threads )
    for ( size_t index_t1 = index_t_start_of_first_bin; index_t1 < index_t_end_of_first_bin; index_t1++ ) {
        ...
        Sparse a_rho_t1 = s.dgl_calc_rhotau( current_state.mat, op_n, identity_matrix, t1 );
        Sparse rho_t1_ad = s.dgl_calc_rhotau( current_state.mat, identity_matrix, op_i, t1 );
        Sparse a_rho_t1_ad = s.dgl_calc_rhotau( current_state.mat, op_n, op_i, t1 );
        Sparse a_rho_t1pT_ad = s.dgl_calc_rhotau( current_state_shifted.mat, op_n, op_i, t1_p_T );

        // Calculate Runge Kutta. For simplicity, we dont even offer the option to use the PI here.

        Sparse eval = op_k * op_l;
        ...
        saved_rho_inner =
            Numerics::interpolate_curve( saved_rho_inner, t1, end_of_fourth_bin + dt, s.parameters.grid_values, s.parameters.grid_steps, s.parameters.grid_value_indices, false, s.parameters.numerics_interpolate_method_tau );
        auto rho_size = saved_rho_inner.size() - 1;
        ////std::cout << "saved_rho_inner.size() = " << saved_rho_inner.size() << ", t start soll " << t1 << ", t end soll " << end_of_fourth_bin << ", t start ist " << saved_rho_inner.front().t << ", t end ist " << saved_rho_inner.back().t << std::endl;
        for ( size_t tau2 = 0; tau2 < index_range; tau2++ ) {
            // Calculate a __ a^+ for t2
            auto t2 = rho_size - index_range - tau2; 
            ////std::cout << "1. rho_size = " << rho_size << ", index_range = " << index_range << ", tau2 = " << tau2 << ", t2 = " << t2 << std::endl;
            Sparse outer = op_m * saved_rho_inner.at( t2 ).mat * op_j;
            calculate_runge_kutta( outer, saved_rho_inner.at( t2 ).t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho, false );
            saved_rho = Numerics::interpolate_curve( saved_rho, saved_rho_inner.at( t2 ).t, end_of_sixth_bin, s.parameters.grid_values, s.parameters.grid_steps, s.parameters.grid_value_indices, false,
                                                     s.parameters.numerics_interpolate_method_tau );
            auto rho_outer_size = saved_rho.size() - 1;
            ////std::cout << "  Time start: " << saved_rho_inner.at( t2 ).t << ", Time end: " << end_of_sixth_bin << ", Time start ist: " << saved_rho.front().t << ", Time end ist: " << saved_rho.back().t << std::endl;
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                auto t3 = rho_outer_size - index_range - tau3;
                auto t3pT = rho_outer_size - tau3;
                //////std::cout << "  1. rho_outer_size = " << rho_outer_size << ", index_range = " << index_range << ", tau3 = " << tau3 << ", t3 = " << t3 << ", t3pT = " << t3pT << std::endl;

                auto &state_at_t3 = saved_rho.at( t3 );
                auto &state_at_t3pT = saved_rho.at( t3pT );

                // EEEEEE -> Calculate Exp. Value for eval
                EEEEEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // EELLEE -> Calculate Exp. Value for eval
                EELLEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // EELEEE -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                EELEEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // EEELEE -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                EEELEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            // Calculate a __ a^+ for t2+T
            auto t2pT = rho_size - tau2;
            ...
            saved_rho = Numerics::interpolate_curve( saved_rho, saved_rho_inner.at( t2pT ).t, end_of_sixth_bin, s.parameters.grid_values, s.parameters.grid_steps, s.parameters.grid_value_indices, false,
                                                     s.parameters.numerics_interpolate_method_tau );
            rho_outer_size = saved_rho.size() - 1;
            ////std::cout << "  Time start: " << saved_rho_inner.at( t2pT ).t << ", Time end: " << end_of_sixth_bin << ", Time start ist: " << saved_rho.front().t << ", Time end ist: " << saved_rho.back().t << std::endl;
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                auto t3 = rho_outer_size - index_range - tau3;
                auto t3pT = rho_outer_size - tau3;
                //////std::cout << "  2. rho_outer_size = " << rho_outer_size << ", index_range = " << index_range << ", tau3 = " << tau3 << ", t3 = " << t3 << ", t3pT = " << t3pT << std::endl;
                auto &state_at_t3 = saved_rho.at( t3 );
                auto &state_at_t3pT = saved_rho.at( t3pT );
                // ELEELE -> Calculate Exp. Value for eval
                ELEELE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // ELLLLE -> Calculate Exp. Value for eval
                ELLLLE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // ELLELE -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                ELLELE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // ELELLE -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                ELELLE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            // Calculate a __ for t2+T
            outer = op_m * saved_rho_inner.at( t2 ).mat;
            calculate_runge_kutta( outer, saved_rho_inner.at( t2 ).t, saved_rho_inner.at( t2pT ).t + dt, timer, progressbar, purpose, s, saved_rho, false );
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                auto t3 = rho_outer_size - index_range - tau3;
                auto t3pT = rho_outer_size - tau3;
                //////std::cout << "  3. rho_outer_size = " << rho_outer_size << ", index_range = " << index_range << ", tau3 = " << tau3 << ", t3 = " << t3 << ", t3pT = " << t3pT << std::endl;
                auto &state_at_t3 = saved_rho.at( t3 );
                auto &state_at_t3pT = saved_rho.at( t3pT );
                // ELEEEE
                ELEEEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // ELLLEE
                ELLLEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // ELLEEE
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                ELLEEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // ELELEE
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                ELELEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                auto t3 = rho_outer_size - index_range - tau3;
                auto t3pT = rho_outer_size - tau3;
                //////std::cout << "  4. rho_outer_size = " << rho_outer_size << ", index_range = " << index_range << ", tau3 = " << tau3 << ", t3 = " << t3 << ", t3pT = " << t3pT << std::endl;
                auto &state_at_t3 = saved_rho.at( t3 );
                auto &state_at_t3pT = saved_rho.at( t3pT );
                // EEEELE
                EEEELE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // EELLLE
                EELLLE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // EEELLE
                Sparse temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                EEELLE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // EELELE
                temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                EELELE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
        }
        ...
        ////std::cout << "saved_rho_inner.size() = " << saved_rho_inner.size() << ", t start soll " << t1_p_T << ", t end soll " << end_of_fourth_bin << ", t start ist " << saved_rho_inner.front().t << ", t end ist " << saved_rho_inner.back().t << std::endl;
        for ( size_t tau2 = 0; tau2 < index_range; tau2++ ) {
            // Calculate a __ a^+ for t2
            auto t2 = rho_size - index_range - tau2;
            ////std::cout << "2. rho_size = " << rho_size << ", index_range = " << index_range << ", tau2 = " << tau2 << ", t2 = " << t2 << std::endl;
            Sparse outer = op_m * saved_rho_inner.at( t2 ).mat * op_j;
            calculate_runge_kutta( outer, saved_rho_inner.at( t2 ).t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho, false );
            saved_rho = Numerics::interpolate_curve( saved_rho, saved_rho_inner.at( t2 ).t, end_of_sixth_bin, s.parameters.grid_values, s.parameters.grid_steps, s.parameters.grid_value_indices, false,
                                                     s.parameters.numerics_interpolate_method_tau );
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                LEEEEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // LELLEL -> Calculate Exp. Value for eval
                LELLEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // LELEEL -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                LELEEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // LEELEL -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                LEELEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            // Calculate a __ a^+ for t2+T
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                LLEELL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // LLLLLL -> Calculate Exp. Value for eval
                LLLLLL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // LLLELL -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                LLLELL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // LLELLL -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                LLELLL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            // Calculate a __ for t2+T
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                auto &state_at_t3pT = saved_rho.at( t3pT );
                // LLEEEL
                LLEEEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // LLLLEL
                LLLLEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // LLLEEL
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                LLLEEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // LLELEL
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                LLELEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            // Calculate __ a^+ for t2+T
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                auto &state_at_t3pT = saved_rho.at( t3pT );
                // LEEELL
                LEEELL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // LELLLL
                LELLLL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // LEELLL
                Sparse temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                LEELLL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // LELELL
                temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho_temp, false );
                LELELL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
        }
        ...
        Sparse inner = op_n * saved_rho_inner.back().mat;
        ...
        for ( size_t tau2 = 0; tau2 < index_range; tau2++ ) {
            // Calculate a __ a^+ for t2
            auto t2 = rho_size - index_range - tau2;
            ////std::cout << "3. rho_size = " << rho_size << ", index_range = " << index_range << ", tau2 = " << tau2 << ", t2 = " << t2 << std::endl;
            Sparse outer = op_m * saved_rho_inner.at( t2 ).mat * op_j;
            calculate_runge_kutta( outer, saved_rho_inner.at( t2 ).t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho, false );
            saved_rho = Numerics::interpolate_curve( saved_rho, saved_rho_inner.at( t2 ).t, end_of_sixth_bin, s.parameters.grid_values, s.parameters.grid_steps, s.parameters.grid_value_indices, false,
                                                     s.parameters.numerics_interpolate_method_tau );
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                EEEEEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // EELLEL
                EELLEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // EELEEL -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                EELEEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // EEELEL -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                EEELEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            // Calculate a __ a^+ for t2+T
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                ELEELL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // ELLLLL
                ELLLLL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // ELLELL -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                ELLELL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // ELELLL -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                ELELLL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                ELEEEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // ELLLEL
                ELLLEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // ELLEEL -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                ELLEEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // ELELEL -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                ELELEL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                EEEELL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // EELLLL
                EELLLL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // EELELL -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                EELELL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // EEELLL -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                EEELLL_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
        }
        ...
        for ( size_t tau2 = 0; tau2 < index_range; tau2++ ) {
            // Calculate a __ a^+ for t2
            auto t2 = rho_size - index_range - tau2;
            ////std::cout << "4. rho_size = " << rho_size << ", index_range = " << index_range << ", tau2 = " << tau2 << ", t2 = " << t2 << std::endl;
            Sparse outer = op_m * saved_rho_inner.at( t2 ).mat * op_j;
            calculate_runge_kutta( outer, saved_rho_inner.at( t2 ).t, end_of_sixth_bin + dt, timer, progressbar, purpose, s, saved_rho, false );
            saved_rho = Numerics::interpolate_curve( saved_rho, saved_rho_inner.at( t2 ).t, end_of_sixth_bin, s.parameters.grid_values, s.parameters.grid_steps, s.parameters.grid_value_indices, false,
                                                     s.parameters.numerics_interpolate_method_tau );
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                LEEEEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // LELLEE
                LELLEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // LELEEE -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                LELEEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // LEELEE -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                LEELEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                LLEELE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // LLLLLE
                LLLLLE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // LLLELE -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                LLLELE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // LLELLE -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                LLELLE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                LEEELE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // LELLLE
                LELLLE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // LELELE -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                LELELE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // LEELLE -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                LEELLE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
            // Calculate a ( __ a^+ ) for (t2+T) t_3(+T)
            ...
            for ( size_t tau3 = 0; tau3 < index_range; tau3++ ) {
                ...
                LLEEEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3.mat, eval, state_at_t3.t );
                // LLLLEE
                LLLLEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( state_at_t3pT.mat, eval, state_at_t3pT.t );
                // LLLEEE -> Iterate a*rho to t3+T, then calculate Exp Value for a^+
                Sparse temp = op_l * state_at_t3.mat;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                LLLEEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_k, saved_rho_temp.back().t );
                saved_rho_temp.clear();
                // LLELEE -> Iterate rho*a^+ to t3+T, then calculate Exp Value for a
                temp = state_at_t3.mat * op_k;
                calculate_runge_kutta( temp, state_at_t3.t, state_at_t3.t + time_bin_length, timer, progressbar, purpose, s, saved_rho_temp, false );
                LLELEE_cache.get( { mat_t_index, index_range - 1 - tau2, index_range - 1 - tau3 } ) = s.dgl_expectationvalue<Sparse, Scalar>( saved_rho_temp.back().mat, op_l, saved_rho_temp.back().t );
                saved_rho_temp.clear();
            }
            saved_rho.clear();
        }
