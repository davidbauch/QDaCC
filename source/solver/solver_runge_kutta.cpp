...
std::pair<Sparse, double> ODESolver::iterateRungeKutta45( const Sparse &rho, System &s, const double t, const double t_step, std::vector<SaveState> &savedStates ) {
    ...
    dSparse errmat = ( drho - ( k1 * e1 + k3 * e3 + k4 * e4 + k5 * e5 + k6 * e6 + k7 * e7 ) ).cwiseAbs2();
...
//TODO: variable time step für phononen!!
bool ODESolver::calculate_runge_kutta_45( Sparse &rho0, double t_start, double t_end, double t_step_initial, Timer &rkTimer, ProgressBar &progressbar, std::string progressbar_name, System &s, std::vector<SaveState> &output, bool do_output, bool interpolate, double tolerance ) {
    ...
    double t_t = t_start + t_step; // + s.parameters.numerics_rk_stepdelta; // + t_step;
    //auto rkret = iterateRungeKutta45( temp.back().mat, s, t_t, s.parameters.numerics_rk_stepdelta, temp );
    Sparse rho;
    while ( t_t <= t_end ) {
        // Runge-Kutta iteration
        ...
        if ( dh > 1 and t_step_new + s.parameters.numerics_rk_stepdelta < s.parameters.numerics_rk_stepmax )
            t_step_new += s.parameters.numerics_rk_stepdelta;
        else if ( dh < 1 and t_step_new - s.parameters.numerics_rk_stepdelta > 0 and t_step_new - s.parameters.numerics_rk_stepdelta > s.parameters.numerics_rk_stepmin )
            t_step_new -= s.parameters.numerics_rk_stepdelta;
        else
            accept = true;
