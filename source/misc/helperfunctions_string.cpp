...
std::string QDACC::String::strip( std::string input, char lit ) {
...
std::vector<std::string> QDACC::String::strip( std::vector<std::string> input, char lit ) {
    std::vector<std::string> ret;
    for ( const auto &str : input ) ret.emplace_back( strip( str ) );
    return ret;
}

std::string QDACC::String::trail( std::string input, int totallength, std::string pre, int dir ) {
    if ( pre.size() == 0 )
        return input;
...
std::string QDACC::String::tail( std::string input, int totallength, std::string pre ) {
    return trail( input, totallength, pre, 1 );
}

bool QDACC::String::startswith( std::string input, std::string find ) {
    return input.substr( 0, find.size() ).compare( find ) == 0;
}

std::vector<std::string> QDACC::String::split( std::string input, std::string lit ) {
    std::vector<std::string> ret;
...
std::string QDACC::String::split_and_reverse( const std::string &input, const std::string &lit ) {
...
std::pair<std::string, std::string> QDACC::String::split_pair( const std::string &input, const std::string &lit ) {
    auto pos = input.find( lit );
    return std::make_pair( input.substr( 0, pos ), input.substr( pos + lit.size() ) );
}

int QDACC::String::instr( const std::string &arr, const std::string tofind, int start ) {
...
std::vector<std::string> QDACC::String::str_to_vec( std::string input ) {
    std::vector<std::string> ret;
    if ( (int)input.size() < 3 ) {
        ret = {};
        return ret;
    ...
    int e = 1; // End
    while ( ( e = QDACC::String::instr( input, ",", s ) ) != -1 ) {
...
int QDACC::String::vec_find_str( const std::string &toFind, const std::vector<std::string> &input ) {
...
std::vector<std::string> QDACC::String::splitline( const std::string &input, const char splitter ) {
...
std::vector<std::string> QDACC::String::argv_to_vec( int argc, char **argv ) {
    std::vector<std::string> ret;
    ret.reserve( argc );
...
std::string QDACC::String::replace( std::string input, const std::string &toreplace, const std::string &replaceto, bool ignore_capitals ) {
    auto pos = input.find( toreplace );
    if ( ignore_capitals and pos == std::string::npos )
        pos = QDACC::String::to_lower( input ).find( QDACC::String::to_lower( toreplace ) );
    if ( pos == std::string::npos ) return input;
    return input.substr( 0, pos ) + replaceto + input.substr( pos + toreplace.size(), input.size() );
}

std::string QDACC::String::add_prefix_and_suffix( std::string input, const std::string &tochange, const std::string &prefix, const std::string &suffix, bool ignore_capitals ) {
    auto pos = input.find( tochange );
    if ( ignore_capitals and pos == std::string::npos )
        pos = QDACC::String::to_lower( input ).find( QDACC::String::to_lower( tochange ) );
    if ( pos == std::string::npos ) return input;
    return input.substr( 0, pos ) + prefix + input.substr( pos, tochange.size() ) + suffix + input.substr( pos + tochange.size(), input.size() );
}

std::string QDACC::String::to_lower( std::string str ) {
