...
class ODESolver {
   ...
   public:
    ...
    Scalar path_integral_recursive( const Sparse &rho0, System &s, std::vector<std::vector<Sparse>> &iterates, std::vector<SaveState> &output, int max_deph, int outer_i, int outer_j, std::vector<int> indices_i, std::vector<int> indices_j, Scalar accumulated_m, FixedSizeSparseMap<Scalar> &adm, bool build_adm, int current_deph = 0 );
    Scalar path_integral_recursive_backwards( const Sparse &rho0, System &s, std::vector<std::vector<Sparse>> &iterates, std::vector<SaveState> &output, int max_deph, int outer_i, int outer_j, std::vector<long long int> indices_i, std::vector<long long int> indices_j, Scalar accumulated_m, FixedSizeSparseMap<Scalar> &adm, bool build_adm, int current_deph = 1E5 );
    Sparse path_integral( const Sparse &rho0, System &s, std::vector<std::vector<Sparse>> &iterates, std::vector<SaveState> &output, FixedSizeSparseMap<Scalar> &adm, bool fillADM, int max_deph );
